...
const Order = () => {
  ...
  const [orderData, setOrderData] = useState(null); // Store order data after creation
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        const cartResponse = await getCart(); // Use the getCart function from CartService
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ flex: '1', padding: '20px', backgroundColor: '#f9f9f9', borderRadius: '8px' }}>
          ...
          cartData.items.map((item, index) => (
            <div key={index} style={{ display: 'flex', marginBottom: '20px', alignItems: 'center' }}>
              ...
              <div style={{ flex: '1' }}>
                ...
                <p style={{ margin: '0', fontSize: '14px', color: '#666' }}>Số lượng: {item.quantity}</p>
              </div>
              <p style={{ fontWeight: 'bold', color: '#C70025' }}>{item.price.toLocaleString()} VND</p>
            </div>
          ))
        ) : (
        ...
        <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #ddd' }}>
          ...
          <p>Phí vận chuyển: {cartData.shippingFee?.toLocaleString() || '0'} VND</p>
          <h3 style={{ color: '#C70025', marginTop: '10px' }}>TỔNG CỘNG: {calculateTotal().toLocaleString()} VND</h3>
        </div>
      </div>
